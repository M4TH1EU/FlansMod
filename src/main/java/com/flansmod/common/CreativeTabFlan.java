...
public class CreativeTabFlan extends CreativeTabs {
	...
	public ItemStack createIcon() {
		...
		switch (type) {
			case 0:
				return GunType.gunList.isEmpty() ? new ItemStack(Blocks.WOOL, 1, 4) : new ItemStack(GunType.gunList.get(icon % GunType.gunList.size()).item);
			case 1:
				return DriveableType.types.isEmpty() ? new ItemStack(Blocks.WOOL, 1, 14) : new ItemStack(DriveableType.types.get(icon % DriveableType.types.size()).item);
			case 2:
				return FlansMod.partItems.isEmpty() ? new ItemStack(Blocks.WOOL, 1, 5) : new ItemStack(FlansMod.partItems.get(icon % FlansMod.partItems.size()));
			case 3:
				return FlansMod.armourItems.isEmpty() ? new ItemStack(Blocks.WOOL, 1, 11) : new ItemStack(FlansMod.armourItems.get(icon % FlansMod.armourItems.size()));
			case 4:
				return FlansMod.mechaItems.isEmpty() ? new ItemStack(Blocks.WOOL, 1, 10) : new ItemStack(FlansMod.mechaItems.get(icon % FlansMod.mechaItems.size()));
			case 5:
